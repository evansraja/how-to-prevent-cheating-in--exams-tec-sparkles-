function num_blocks=hog2features(hog_c, patch_dims)
...
global g_hog_blocks;
if ~config.USE_PHOG
   num_blocks=hog2features_internal(hog_c, patch_dims);
else
   [num_blocks1,hog_blocks1]=hog2features_internal(hog_c.hog1, patch_dims);
   ...
   for y=1:H2
       for x=1:W2
           [nb,hb] = hog2features_internal(reshape(hog_c.hog2(y,x,:,:,:),Y2,X2,D), patch_dims/2);
   ...
   for y=1:H4
       for x=1:W4
           [nb,hb] = hog2features_internal(reshape(hog_c.hog4(y,x,:,:,:),Y4,X4,D), patch_dims/4);
